...
// 修复：CMakeLists 增加 -fno-threadsafe-statics——static 局部初始化不再生成
//       __cxa_guard_acquire/release 调用（直接顺序初始化），入口路径零 PLT：
//       - static Api/module_abi 构造为成员赋值 + 无捕获 lambda（零外部符号）
//       - registerModule/onLoad 为函数指针/虚调用（经 RELRO RELATIVE 重定位）
//       - 单线程窗口（zygote 主线程首次调用）下无竞态，语义等价
// 备注：曾尝试自定义入口（全局对象 + 栈上 abi），但 Api::tbl 为 private 成员
//       （仅 entry_impl friend 可访问），自定义入口无法赋值 → 放弃，flag 方案更简。
