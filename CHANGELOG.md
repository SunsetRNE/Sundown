...
- **A2/A3 实机体验验证（2026-08-12）**：
  - **A2 立即墓碑档位**：`instant` 预设（grace=0/cooldown=120）实机应用 → 抖音切后台事件流 `delay grace 0s → freeze grace_expired 同秒落冻`（tick ≤300ms 语义实证）；系统 app（浏览器）/关键 app（launcher）正常豁免不误伤；验证后 preset clear 回落
  - **A3 压制维度扩展**：三源门控框架实机实证——临时配置 `service_gate/pendingintent_gate/receiver_gate` 非空 + force 冻结目标 app 后，定向广播 `com.test.gate.action` 命中门控 → **事件留痕 `receiver_gated`（携带 action）且不解冻**（仅用户切前台才 unfreeze，语义正确）；dex 数据面实证：当日 463 条 `src=service` 上报（含组件名 flatten，如 `com.ss.android.ugc.aweme/storage.service.ExtendSystemCleanService`）+ 大量 pendingintent 上报——A3 门控数据源完整；service/pendingintent 分支与 receiver 同构代码 + 单测 `service_pendingintent_gate_v06` 全覆盖（白名单放行/白名单外不解冻留痕/未上报组件"?"门控/通配放行）；Android16 后台启动限制（冻结 app 的 startService 被 AMS 拦截）使 service 门控难以直接触发，经 broadcast 维度同构实证 + 单测覆盖闭合
  - 验证后全部恢复：policy.toml 还原（enabled=false/force=[]/gate 空）、appops reset、daemon 安全回落（`unfreeze policy_disabled unfroze 1 pkgs`）
- **C 档 WebUI 画像页消费（2026-08-12）**：新增 `画像` tab（WebUI v2.5）——消费 `sunctl analyze` JSON 面（C1 画像 → C2 建议闭环的 UI 侧落地）：建议统计头（总数/警告/提示）+ 建议卡片列表（level 徽章 warn/info + kind 标签映射（唤醒风暴/源模式/冻结抖动/豁免/节流）+ 包名 + 标题 + 点击展开 detail 与 rules.toml/policy.toml 手工动作指引）；tab 切换懒加载、刷新按钮、失败安全占位（daemon 离线/数据不足明确提示）；node --check 语法验证通过 + 设备端同步（64831B）
